...
def write_yolo_polygons(
    txt_path: Path, polygons: list[tuple[int, np.ndarray]], img_shape: tuple[int, int]
) -> None:
    h, w = img_shape[:2]
    lines = []
...
def rotate_image_and_mask(
    img: np.ndarray, mask: np.ndarray, angle_idx: int
    ...
    if angle_idx == 1:
        ...
        return cv2.rotate(img, cv2.ROTATE_90_COUNTERCLOCKWISE), cv2.rotate(
            mask, cv2.ROTATE_90_COUNTERCLOCKWISE
        )
    elif angle_idx == 4:
        ...
        return cv2.rotate(cv2.flip(img, 1), cv2.ROTATE_90_CLOCKWISE), cv2.rotate(
            cv2.flip(mask, 1), cv2.ROTATE_90_CLOCKWISE
        )
    elif angle_idx == 7:
        return cv2.rotate(cv2.flip(img, 1), cv2.ROTATE_90_COUNTERCLOCKWISE), cv2.rotate(
            cv2.flip(mask, 1), cv2.ROTATE_90_COUNTERCLOCKWISE
        )
    return img, mask
...
def process_dataset(split: str = "train"):
    ...
    print(
        f"  > Generados {total_crops_generated} tiles "
        f"(incluyendo {oversampled_count} copias extra)."
    )
